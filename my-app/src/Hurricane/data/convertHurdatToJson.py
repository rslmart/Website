...
def addWindPolygons(track_point):
    ...
    for key in key_list:
        if track_point[key] > 0:
            ...
            poly_cords = [coords, bot_top_coords, new_coords, left_right_coords, coords]
            for coord in poly_cords:
                # Adjustments for west pacific coordinates
                if coord[0] > 100:
                    coord[0] = coord[0] - 360
            track_point[key + "_poly"] = poly_cords
...
if __name__ == '__main__':
    file_names = ["hurdat2.txt", "hurdat2_nepac.txt"]
    storms = {}
    track_points = []
            ...
            for line in file:
                ...
                if len(line_arr) == 4:
                    ...
                    try:
                        ...
                        # Adjustments for west pacific coordinates
                        if track_point["longitude"] > 100:
                            track_point["longitude"] = track_point["longitude"] - 360
                        storms[storm_id]["track_points"].append(track_point)
                        track_points.append(track_point)
                    except Exception as e:
                        raise type(e)("Line: {}".format(line_arr)).with_traceback(e.__traceback__)
