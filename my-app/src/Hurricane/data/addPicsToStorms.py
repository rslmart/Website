...
if not os.path.exists('./pics.json'):
    with open('atlcpacepac_ir_geo_1km.csv', 'r') as read_obj:
        ...
        if season in pic_dict and basin in pic_dict[season] and name in pic_dict[season][basin]:
            for point in storm_dict[storm_id]["track_points"]:
                ...
                for delta in range(15, 3*4*15, 15):
                    matching = [p for p in pic_dict[season][basin][name] if convertDTStr(p[0]) >= point_date - timedelta(minutes=delta)
                                and convertDTStr(p[0]) <= point_date + timedelta(minutes=delta)]
                    if len(matching) > 0:
